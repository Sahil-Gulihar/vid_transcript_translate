...
# --- Get YouTube link from command line argument
if [ -z "$1" ]; then
...
# 1. Download audio from YouTube using yt-dlp and ffmpeg
echo "--- Downloading audio from YouTube ---"
yt-dlp -x --audio-format flac -o "${FILENAME}.%(ext)s" "$YOUTUBE_LINK"
...
# 3. Translate transcription to Hindi using Gemini Flash
